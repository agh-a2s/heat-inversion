...
function [C, dCdK] = heat_with_gradient(K1, K2, K3)
    % Problem formulation
    ...
    [L, U, P, Q] = lu(M);
    invM = @(b) Q * (U \ (L \ (P*b)));
    u = solve_heat_with_matrix(invM, f, g, fixed_dofs, bx, by, k);
    toc
    ...
    % % FD approximation
    % solution = @(k1, k2, k3) solve_heat(a(k1, k2, k3), f, g, fixed_dofs, bx, by, k_rhs);
    % h = 1e-3;
    %
    % dCdK = zeros(N, 3);
    % tic
    ...
    %     Q = uniform_measure(measured_points(i,:), radius);
    %
    %     C_next = quantity(solution(K1 + h, K2, K3), Q, bx, by);
    %     C_prev = quantity(solution(K1 - h, K2, K3), Q, bx, by);
    %     dCdK(i,1) = (C_next - C_prev) / (2*h);
    %
    %     C_next = quantity(solution(K1, K2 + h, K3), Q, bx, by);
    %     C_prev = quantity(solution(K1, K2 - h, K3), Q, bx, by);
    %     dCdK(i,2) = (C_next - C_prev) / (2*h);
    %
    %     C_next = quantity(solution(K1, K2, K3 + h), Q, bx, by);
    %     C_prev = quantity(solution(K1, K2, K3 - h), Q, bx, by);
    %     dCdK(i,3) = (C_next - C_prev) / (2*h);
    %
    %     fprintf("FD dC%d/dK = [%f, %f, %f]\n", i, dCdK(i,:));
    % end
    % toc
    % FD = dCdK(:,:);
    ...
    for i = 1:N
        ...
        dCdK(i,1) = eval_form(a(1,0,0), u, lambda, bx, by);
        dCdK(i,2) = eval_form(a(0,1,0), u, lambda, bx, by);
        dCdK(i,3) = eval_form(a(0,0,1), u, lambda, bx, by);
        fprintf("AS dC%d/dK = [%f, %f, %f]\n", i, dCdK(i,:));
    end
    toc
    %AS = dCdK(:,:);
    % diff = AS - FD;
