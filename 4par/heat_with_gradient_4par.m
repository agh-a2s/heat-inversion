...
function [C, dC] = heat_with_gradient_4par(K1, K2, K3, s, f, df, g, measured_points)
    % Parameters:
    % K1, K2, K3 - heat conductivities on subdomains (positive scalars)
    % s - source range (positive scalar, strictly less than 1)
    % f - source function
    % df - source function derivative with respect to s
    % g - Neumann condition on edges not incident to (0,0)
    % measured_points - points where QOIs are computed
